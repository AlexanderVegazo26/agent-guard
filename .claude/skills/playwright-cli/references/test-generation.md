...
Plan / generate / heal lean on the same mechanic: run `npx playwright test --debug=cli` in the background, then `npm run test:cli -- attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics.
...
```bash
# Start a session
npm run test:cli -- open https://example.com/login
...
Use `npm run test:cli -- generate-locator <target>` to produce the locator expression for the assertion, and the snapshot/eval commands to capture the expected value.
...
npm run test:cli -- --raw generate-locator e5
# getByRole('button', { name: 'Submit' })

# Capture expected text content for toHaveText
npm run test:cli -- --raw eval "el => el.textContent" e5

# Capture expected input value for toHaveValue/toBeEmpty
npm run test:cli -- --raw eval "el => el.value" e5

# Capture expected aria snapshot for toMatchAriaSnapshot/toBeChecked
# (whole page, or use a ref to scope to a region)
npm run test:cli -- --raw snapshot
npm run test:cli -- --raw snapshot e5
```
...
npm run test:cli -- attach tw-XXXX
```

Resume so the seed runs, then probe the app:

```bash
npm run test:cli -- resume                   # resume so that seed test runs fully
npm run test:cli -- snapshot                 # inventory of interactive elements
npm run test:cli -- click e5                 # follow a flow
npm run test:cli -- eval "location.href"     # read URL / state
npm run test:cli -- show --annotate          # ask the user to point at something
```
...
```bash
npm run test:cli -- snapshot                         # find refs
npm run test:cli -- fill e3 "John Doe"               # -> page.getByRole('textbox', {...}).fill(...)
npm run test:cli -- press Enter
npm run test:cli -- click e7
```
...
```bash
npm run test:cli -- snapshot                # did the element change / move / rename?
npm run test:cli -- console                 # app-side errors?
npm run test:cli -- requests                # failed request? wrong payload?
npm run test:cli -- show --annotate         # ask the user to point somewhere
```
