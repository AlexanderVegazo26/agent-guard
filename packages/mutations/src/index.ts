/**
 * `@alexvegman/mutations` — PRD3 F14 / A4: the mutation catalogue as a
 * package with a registry, separate from `HttpFaultProxy`'s mechanism
 * (`@alexvegman/observe`) and from `FaultSpec` itself, which stays in
 * `@alexvegman/core` because `AgentRun.faults` already references it and
