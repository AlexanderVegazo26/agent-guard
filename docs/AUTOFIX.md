...
implemented and remains blocked on §4.2** (per-assertion held-out fixture
sets — needs real fixture volume this repo doesn't have yet) — there is
no `autofix validate` and no `autofix apply` command, and neither should
be built until that's closed.

**2026-09-19 update (PRD2 F10):** §4's other two prerequisites are now
real code, not just prose. §4.1's mechanical half — every fixture's
`mustCite` evidence ids actually exist in its compiled evidence graph —
is `auditMustCite()` (`@agent-guard/core`), wired into
`agentguard audit-fixtures`, run clean against all 30 fixtures in this
repo, and confirmed to catch a deliberately introduced break. It is
explicitly **not** the full semantic audit this section describes below
(see §4.1's own text, unchanged) — it cannot tell whether cited evidence
actually *discriminates* a verdict, only whether it exists. §4.3's
baseline-variance floor is `computeVariance()`/`exceedsVarianceFloor()`
(`@agent-guard/core`), real measurement code, not yet wired into
`agentguard compare` or any autofix command. Every proposal `autofix
show` prints is still labeled `NOT VALIDATED`, correctly — §5 itself
remains unbuilt, and building it means running it live, which this pass
does not do without separate, explicit spend authorization.
